...
.^|xref:IZCloud:DocList.adoc[IZCLOUD-CAMP-MAN-002a] |{software-cloud} for Campgrounds User Guide
.^|xref:SLN-GKSYS:DocList.adoc[GKSYS-MAN-002a] |{compname-short} GKSYS User Guide
.^|xref:SLN-GKSYS:DocList.adoc[GKSYS-MAN-002b] |{compname-short} GKSYS Installation and Calibration Guide
.^|xref:SLN-TIBA:DocList.adoc[TIBA-MAN-002a] |{compname-short} TIBA User Guide
.^|xref:SLN-TIBA:DocList.adoc[TIBA-MAN-002b] |{compname-short} TIBA Installation and Calibration Guide
|===
...
This document is applicable to {docproductname}
software, release 1.15.120 and above.
...
{shortprodname} is advanced ALPR software
that reads license plates using
sophisticated image recognition algorithms,
and creates LPR Events. {shortprodname}
sends LPR Event data via HTTP,
the proprietary {compname-short}
{software-protocol} protocol
or other protocols to other systems,
typically {software-server}
(see the {software-server} User Guide
- see <<s_Related-Documents>>). {shortprodname} can be configured and monitored using a web-based GUI.
...
. You will see the dialog used for changing
...
The Trigger Operational Modes (set in the Detector Configuration section
of the Settings tab - see <<s_Detector-Configuration>>) affect how Events are created (see <<f_Trigger-Operational-Modes>>):
...
image::ROOT:RoadViewALPR/RV-1_15-FIG-100c_SettingsVersionSection.png[image,width=356,height=130]
...
{shortprodname} can send Events via HTTP, and act as
both a {software-protocol} Server and {software-protocol} Client - all simultaneously.
...
[NOTE]
================================
If {software-cloud} Integration is enabled, the Access Lists tab
becomes visible. See <<s_Access-Lists-Tab>>.
================================
...
=== GKSYS (GateKeeper) Integration

See the GKSYS document (see <<s_Related-Documents>>).
...
image::./UserGuide/RV-1_15-FIG-206a_ZAP_Integration.png[image,width=393]
...
(Requires Offline to be Enabled in the {software-protocol} section)
+
...
** *Keep Alive Enabled* - Send a GetStatus message every Keep Alive Rate (see
Keep Alive Rate parameter)

** *Keep Alive Rate (milliseconds)* - The interval at which GetStatus messages
are sent (see Keep Alive Enabled parameter)
...
** *Offline Send Rate* - How often offline Events are sent from the offline queue
...
*** +++<u>+++Always FTP Paths+++</u>+++ - Always include FTP paths
to images in Event messages and EventImages messages

** *HTTP Paths* - (appears if "FTP Enabled" is not checked)
If this parameter is enabled, {software-ALPR} sends an HTTP URL of each image via
{software-protocol}, instead of the path on the FTP server or the path
on the local drive. You can use these URLs to retrieve and display images
from {software-ALPR}.
...
** *FTP Enabled (for {software-protocol} Client)* - When
{shortprodname} is connected to a {software-protocol} server,
images can be sent to the server via FTP if this checkbox is checked.
(See the FTP Server items for the definition of the additional FTP
parameters.)

** *HTTP Paths* - (See the FTP Server items for the definition of this parameter.)
...
Send all LPR and OV images, even if there was no plate
recognition - plus all LPR and OV ROIs, if there was
plate recognition
...
* *All OV, all OV ROI, all LPR, all LPR ROI* - Send all LPR
and all OV images
...
This parameter is used for Camera System models
with external trigger input wires, such as the {camera-5G}.
See your Camera System's Installation Guide for details.

A loop controller can provide trigger information to {shortprodname}.
You can connect dry or wet contacts to the Camera System's
trigger inputs. The rise and fall of the voltage levels
at these inputs sends a trigger to {shortprodname}
(see <<s_Trigger-Sources-and-Trigger-Modes-Overview>>).
...
The Journal (Local Storage) parameters determine if and how
Events are stored on the local {shortprodname} computer disk
(the parameters only appear if the disk is in use).
...
If the local {shortprodname} computer disk is not accessible,
then the Journal will be stored at: +
/home/izuser/jsoft/data/journal
...
[#f_Use-Validation-Section]

.Use Validation Section

image::ROOT:RoadViewALPR/RV-1_15-FIG-217_UseValidation.png[image,width=477,height=85]

Use Validation provides additional functionality to improve read accuracy,
readjust confidence and accommodate custom site needs.

* *LPR Threshold* - The LPR read confidence level above which further validation will be applied.

* *State Threshold* - The State read confidence level above which further validation will be applied.

The conditions required for further validation to be applied are:

* The confidence of the LPR and State initial reads must BOTH be
above the Use Validation thresholds.

* The Region selected in the Detector Configuration section (see <<s_Detector-Configuration>>)
must be supported in the list of validation criteria patterns, called "plate types" (set by {compname-short}).

* The LPR and State reads must both match to one of the validation criteria patterns.

If all of the conditions are met:

* The "plate type" (internal {compname-short} designation) can be identified

* Any actions associated with this "plate type" can be applied to the read
(for example, removal of a trailing character)

* An abbreviation of the "plate type" will be attached to the Event so
it can be displayed on the Live (Journal) and Search tabs
(see <<s_Live-Journal-Tab>> and <<f_Configure-Journal-Multi-line-Menu>>). For
example, "BTR" indicates a B-type truck in certain states.

Exceptions:

* If the Region selected in the Detector Configuration section
is not supported in the list of validation criteria patterns,
then no further validation is performed, and the initial LPR
and State confidences are left as they were.

* If the Region selected in the Detector Configuration section
is supported, but either the LPR or State confidences are below the
Use Validation thresholds, then the initial
LPR and State confidences will be reduced as follows and
reapplied to the Event:

** LPR confidence will be reduced by 10%

** State confidence will be reduced by 50%

* The validation list should contain all LPR{plus}State
variations. If the Region is supported, and the LPR/State confidences
are above the thresholds, but there was no match with one of the validation patterns,
this indicates an error in the LPR or State read. The initial
LPR and State confidences will be reduced as follows and
reapplied to the Event:
...
* *Include all images* - Enables display in Live tab, and sending of +++<u>+++all+++</u>+++ images (including pre- and post-trigger frames) - not only the "best" ones that were used for plate recognition
...
click the Save button in the Camera Add/Edit dialog
(even if you did not change the URL),
you may see a warning icon in the Image column.
The reappearance of the image indicates that the core software
has restarted, and Events will resume being captured and
displayed in the Live tab.
...
If you return to the Live tab from another tab, the large
picture returns to the OV (color) camera image.

DOT (USDOT number image capture) cameras generate Events, but without LPR reads.

========================================

* Each row includes (additional items
can be added from the multi-line menu icon
> Configure Journal selection -
see <<s_Live-Journal-Tab>>):

** +++<u>+++Transaction (Event) ID+++</u>+++. Note that
each Event has a unique ID, regardless of which camera
it came from.
...
*** Select "Configure Journal" (see <<f_Configure-Journal-Multi-line-Menu>>)
+
...
*** Select the additional columns to display, such as: +++<u>+++State+++</u>+++ (state displayed on plate), +++<u>+++State Confidence+++</u>+++ (confidence that the state has been recognized accurately), +++<u>+++Class+++</u>+++ (vehicle class, such as car or truck), +++<u>+++Class Confidence+++</u>+++ (confidence that the class has been recognized accurately), +++<u>+++Plate Type+++</u>+++ (determined by the Use Validation feature - see <<s_Use-Validation>>)
and +++<u>+++Direction+++</u>+++ (direction vehicle was traveling - forward or backward - according to the direction arrow configured in the Calibration tab - see <<s_Calibration-Tab>>). (These additional columns will also appear on the Search tab - see <<s_Search-Tab>>.)
...
image::ROOT:RoadViewALPR/RV-1_15-FIG-205b_SystemStatus.png[image,width=400]
...
include::ROOT:partial$RoadViewALPR/p-RoadViewALPR-calibration-tab-text.adoc[]
...
image::ROOT:RoadViewALPR/RV-1_15-FIG-034e_SystemInfo.png[image,width=400]
...
image::ROOT:RoadViewALPR/RV-1_15-FIG-218_AccessLists.png[image,800]

[NOTE]
================================
The Access Lists tab only becomes visible when {software-cloud}
Integration is enabled. See <<s_IZCloud-Integration>>.
================================

When {software-ALPR} starts or when {software-cloud} access lists are updated,
{software-ALPR} downloads the latest access lists and stores them on disk.
This enables {software-ALPR} to continue sending signals to open gates
for authorized vehicles, even if the connection to {software-cloud}
has been lost.

{software-cloud} can have several different access lists to grant or deny
access (such as Whitelist and Blacklist).
See <<s_Related-Documents>> for {software-cloud} documentation.

These lists are applied to all {software-cloud}
physical lanes. However, each vehicle owner's
access to the site can be limited to
certain areas. A "lane" on the {software-ALPR}
Access Lists tab represents a camera
viewing an area of a site that the vehicle owner has
access to. For this reason, each "lane" can have its own set of "lists" (for example, lane 1 and lane 2
can each have their own "Grant" list.

To display a list, select a Lane and List from the dropdowns.
Due to a site's business scenarios, plates can
appear more than once in a list.

* The Update Time at the top of the tab is the
last time that {software-ALPR} received an update
from {software-cloud}.

* The Plate is the plate stored in the list.

* The Start and End times are the times
configured in {software-cloud} for which
the vehicle will be acted upon (to grant or deny access).

* You can enter characters in the filter boxes at the top
of each column to find within the strings. For example,
KZ will find **+++<u>+++KZ+++</u>+++**R3791 and
J**+++<u>+++KZ+++</u>+++**0714.
...
Doc. No. RV-ALPR-MAN-001 Ver. {docdate}
