...
must be supported in the list of validation criteria (set by {compname-short}).

* The LPR and State reads must both match to one of the validation criteria.
...
A "lane" on the {software-ALPR}
Access Lists tab represents a camera
viewing an area of a site that the vehicle owner has
access to. For this reason, each "lane" can
have its own set of "lists".

In addition, due to a site's business scenarios, plates can
...
last time that {software-ALPR} received and update
from {software-cloud}.
