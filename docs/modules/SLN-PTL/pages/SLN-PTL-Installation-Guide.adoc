...
image::ROOT:SLN-PTL/MOTOROLA_LOGO.png[image,150,align=left]
...
. Allocate an IP address for
the {shortprodname} Data Processing Unit (DPU).
For example: +
...
. Type in the IP address of the {software-ALPR} computer - the {shortprodname}.
For example: +
...
For example, if the {shortprodname} was moved
...
* If needed, set the {shortprodname} IP address parameters.
