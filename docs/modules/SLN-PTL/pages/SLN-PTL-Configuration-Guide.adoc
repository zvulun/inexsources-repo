...
a web-based GUI and an API. Some parameters can only be configured
via the API,  others only via the GUI, and some
via both (see *_\{zvulun - xref to table
converted from Max's table}_* ).
See the  API document for further details.

[table.withborders,width="100%",cols="15%,30%,10%,10%,10%,25%",options="header",]
|===

|Name |Description |API Controlled |GUI Controlled |Local Storage |Notes
