...
* *Camera Port* - the HTTP port on which {software-ALPR}
listens for commands from the {docproductname3} server

* *Log Store Days* - how long logs are stored for
...
Events sent to the {docproductname3} server
...
*_\{Zvulun tell users to enable journal and use defaults}_*
...
*_\{Zvulun: mention video link but not metadata link}_*
