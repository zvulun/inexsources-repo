...
Data Processing Unit (DPU) can be found on the sticker on the device;
typically {ip-default-IZA-DPU}.

. If you need to change the factory default address,
use {software-discovery} to change its IP address. Be sure that
you have allocated this address in your network.
...
image::ROOT:SLN-TIBA/RV-TIBA-FIG-007a_EditLaneDialog.png[image,150]
...
can he do it manually? or maybe he can't?}_* zoom to have one camera "see" closer than the other one.
