...
== Solution BOM
*_\{is this list of models/countries still correct for GKSYS??}_*
...
image::ROOT:SLN-GKSYS/RV-GKSYS-FIG-003b_Trigger_Offset.png[image,600]
...
** *_\{delete this?}_* +++<u>+++Triggered+++</u>+++ - Events are created and reported when a trigger is received
...
* *_\{delete this bullet?}_* *Include all images (for "Triggered" and "NonTriggered With Trigger" modes only)* - Enables display in Live tab, and sending of +++<u>+++all+++</u>+++ images (including pre- and post-trigger frames) - not only the "best" ones that were used for plate recognition
