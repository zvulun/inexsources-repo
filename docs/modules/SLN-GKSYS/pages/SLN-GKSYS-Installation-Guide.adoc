...
*_\{should we put the instructions in for
connection to the Cisco client - and if so, at
what point in this doc? - see emails describing how
this is done. Also, where will the user get the Cisco
Anyconnect installer software - from "your system
manager? From Inex?}_*

This document describes how to install,
configure and calibrate an {compname-short} {generic-camera-name}
for a *_\{single lane is correct?}_* single lane, to work with the {docproductname} {docproductname3} System.
...
.^| *_\{should be 40 based on IZA500G/GR comparison ppt?}_* Up to 80 mph (129 km/h)

2+.^| LPR Capture Distance (D) +
(for U.S.A. plates; +
measured from camera to plate)

.^| *_\{change according to IZA500G/GR comparison PPT?}_* 5-70 ft (1.5-21 m); 8 ft (2.4 m) recommended

|===

*_\{are these pics correct or do they need changing?}_*
...
*_\{need settings for the GKSYS project -
these were taken from TIBA}_*
...
and password (root, root): *_\{change to dialog with reset password link}_*
+
...
* Select "Logout" *_\{need picture with Change Password option}_*
...
*_\{review all of these screens to see if they are
correct for GKSYS}_*
...
*_\{need new picture showing 1.15.120; also, is
it a problem if the main RV user guide shows
an IZA500G (not GR) in its examples?}_*
...
*_\{change location to a u.s. location
like new york - not Israel?}_*
...
*_\{is this relevant?}_* The minimum {docproductname} {docproductname3} version
required is 9.3.3
...
port. For example: +
\https://198.132.128.24:8892/api/LicensePlateStatus
...
plate read matches to the {docproductname} list of expected
...
*_\{take a new screen shot with the Detector
Configuration setting for GKSYS}_*
...
*_\{need new screen shot for GKSYS}_*
...
*_\{use a screen shot from the actual
GKSYS site}_*
...
*_\{Is IZCloud relevant for GKSYS? and if so,
is the version of the IZCloud screens shown here
correct for GKSYS - these are the screens from TIBA}_*
...
*_\{recheck this table to see if it is relevant for GKSYS}_*
