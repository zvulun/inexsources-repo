...
generate an alert/alarm (see <<s_Alerts-Alarms>>). You can add or remove
...
that has generated an alarm (see <<s_Alerts-Alarms>>).In addition, the number of
...
the Events page (see <<s_IZCloud-Events>>).
...
* Add the license plate to an Access list
