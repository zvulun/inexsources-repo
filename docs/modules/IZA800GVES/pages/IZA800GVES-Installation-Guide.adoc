...
When using {compname-short} cables (such as the IZ Composite Cable or the prefabricated Power/LAN Cable), for proper LAN connectivity from the Camera System via the {hw-connection-panel} (see <<s_Connecting-the-IZ-Panel>>), the maximum cable length is 200 feet.
...
* LAN - When using {compname-short} cables (such as the IZ Composite Cable or the prefabricated Power/LAN Cable) for LAN connectivity from the Camera System via the {hw-connection-panel}, the maximum cable length is 200 feet.
...
* If you have an IZ Composite Cable, you will need to connect one end to the connector plug supplied with the {shortprodname} Camera System (see <<s_Connecting-the-IZ-Composite-Cable-to-the-Power-LAN-Connector-Plug>>).
...
If you are using an illuminator cable, secure or remove
the Camera System's connector protective cap to prevent it from falling (safety hazard).
...
. Pull the shield mesh and plastic jacket
back along the cable to expose the drain wire.
The drain wire is the same color as the shield mesh,
and is composed of several strands wound together
(see <<f_Locating-the-Drain-Wire>>).
+
[#f_Locating-the-Drain-Wire]

.Locating the Drain Wire

image::image22.png[image,width=385]
...
Be sure to tighten the sleeve just enough to feel a little resistance. This will enable the connector plug to be rotated more once it is connected to the Camera System (see <<f_Connector-Plugs-with-Cables-Attached-to-Camera>>).
...
. Once you have finished tightening the sleeve, turn the hood to the desired angle which for which you want the cable to leave the Camera System (see <<f_Connector-Plugs-with-Cables-Attached-to-Camera>>).
...
Turn off/disconnect the external (AC) power supply
before connecting cables. *If you are using an
{compname-short} power supply,
see its User Guide (see <<s_Related-Documents>>)
...
When using {compname-short} cables (such as the IZ Composite Cable) for LAN connectivity from the Camera System via the {hw-connection-panel}, the maximum cable length is 200 ft (61 m).

If you are using an illuminator cable, secure or remove
the Camera System's connector protective cap to prevent it from falling (safety hazard).

To attach the cables to the Camera System, tighten the outer insert ring onto the corresponding panel connector (see <<f_Female-Connector-Plug-Assembly>>).

After connecting the cables to the Camera System, you can still rotate them slightly to achieve the desired angle (see <<f_Connector-Plugs-with-Cables-Attached-to-Camera>>). If you need a bigger rotation, see <<s_Connecting-the-IZ-Composite-Cable-to-the-Power-LAN-Connector-Plug>>.
...
The {hw-connection-panel} is used in connection cabinets.
It provides a convenient way to connect the Power/LAN
cable from the {shortprodname} Camera System to the LAN and power supply (see the {hw-connection-panel} documentation referred to in <<s_Related-Documents>>).
...
* Once the lane is active, verify that Events are being
generated for each vehicle passing each Camera System, and
that the recognition has sufficient accuracy and confidence.
