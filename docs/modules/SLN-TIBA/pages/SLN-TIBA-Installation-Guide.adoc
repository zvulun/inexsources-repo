...
A trigger command may arrive at a different
time than actual trigger’s (physical) arrival
time. For example, there is often a delay between
the time a vehicle passes over an inductive loop,
and the time the loop controller generates a pulse.
This latency can be compensated for by an "offset".
