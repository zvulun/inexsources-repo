...
plate reader confidence}_* - see <<s_Detector-Configuration>>).
...
* *Require Maintenance* - *_\{please verify}_* Lanes with cameras
that are operating but need maintenance

* *Inoperational Lanes* - Lanes with
cameras not operating

* *Read Rate Bar Chart* (for online/operational cameras) -
Hover over a bar to show details.
*_\{only shows a small amount of Review Events Count.
What data is used and over what time period to
generate the bars?}_*
...
image::ROOT:SLN-TIBA/IZCloud-BorderTIBA-Lanes.png[image,800]
...
to correct the read.  *_\{please verify}_* A new Event is created
