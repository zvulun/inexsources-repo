...
If you want to capture a vehicle
(and create an Event) at a different
position than the location of the loop
(before or after), this can be achieved by
setting the Trigger Offset parameter.

the image from the Lane 1 TIBA –
loop is directly under the barrier bar,
“-200” will move capturing point upstream
on the lane and capture the plate before
