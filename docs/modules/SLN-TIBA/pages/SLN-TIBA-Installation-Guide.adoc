...
are on the unit's sticker typically: Processor ({ip-default-IZA-DPU}), LPR Camera ({ip-default-IZA-LPR})
OV Camera ({ip-default-IZA-OV}). If needed, use {software-discovery} to change to other
+++<u>+++consecutive+++</u>+++ addresses allocated in your network.
