...
*_\{Pinchas: since the table is here
...
in the automatic read *_\{Pinchas: or is it
an automatic read that does not
match the true read?}_* an automatic read
with a low confidence below an acceptable
threshold *_\{Pinchas: is this correct?}_* (as set in the {software-ALPR} Settings tab,
in the Detector Configuration section
*_\{Pinchas: which parameter - detector confidence or
plate reader confidence}_* - see <<s_Detector-Configuration>>).

** *False Reject* - *_\{Pinchas: please verify}_* an
