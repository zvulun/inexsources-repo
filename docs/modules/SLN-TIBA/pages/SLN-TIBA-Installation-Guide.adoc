...
*_\{since the table is here
should I remove the distance
recommendations in the lane design
drawings?}_*
...
.^|A .^a|*LAN Patch Cable(s)* - CAT6, max. length 3.3 ft (1 m) .^|Supplied by integrator
.^|B .^a|*LAN Cable(s)* - CAT6, 4 pairs, 26 AWG, max.
length 328 ft (100 m), OD 6.5-7.0 mm
.^|Supplied by integrator; 6.6 ft (2 m) included with camera
.^|C .^a|*PAS (Power and Signals) Cable* -  22-26 AWG, OD 7.0-7.2 mm
...
* *Resident* -
*_\{should I call this "unused"? or is it
the vehicle owner name; Enter characters
to find within the names? first names? }_*
...
** *False Read* - *_\{or is it
an automatic read that does not
match the true read?}_* an automatic read
with a low confidence below an acceptable
threshold *_\{is this correct?}_* (as set in the {software-ALPR} Settings tab,
in the Detector Configuration section
*_\{which parameter - detector confidence or
plate reader confidence}_*
- see <<s_Detector-Configuration>>).

** *False Reject* - *_\{please verify}_* an
automatic read that should not have been
rejected due to a low confidence
...
Maintenance Dashboard - HIstorical Avg Read rate
is daily report - hover on a dot to get details
Use Gate dropdown to select which camera
only write about lanes from campgrounds
or not at all
You can click on bar chart - not just hover -
to get to the Last Review screen pre-filtered
for that Lane; click on bars
(only for online cameras) of read rate to
show the list of reads used for that camera
to get the statistics}_*

[#s_IZCloud-Lanes]

=== {software-cloud} Lanes

The Lanes page enables you to:
* View live video
* View the license plate read for the current Event
* Correct the plate read *_\{does this change
the plate read, or just put the correction into the
true read field (Plate Review on the Last Review page)}_*
...
to *_\{does this change
the plate read, or just put the correction into the
true read field (Plate Review on the Last Review page)}_* correct the read +
