...
.^|xref:IZ4POE:DocList.adoc[{hw-poeinjector}-DIN-REV-xxx-TDSHEET] |{hw-poeinjector} DIN Rail Mount Gigabit PoE{plus}{plus} Injector Technical Data Sheet
...
|===
|Item |Notes
|24 VDC (or 24/48 VDC for P24/P48 models using DC input) power supply (voltage-adjustable) |{compname-med} model power supply. (If you use an external illuminator, it is recommended to use an illuminator power supply separate from the Camera System's supply.)
|Gigabit PoE{plus}{plus} Injector | Required for P24/P48 models when using PoE+ power input
