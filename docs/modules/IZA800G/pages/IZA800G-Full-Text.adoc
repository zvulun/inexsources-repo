...
image:ROOT:image$/IZA500G/IZA500G-FIG-001e_FrontPagePhoto.png[Picture of Camera,400,align=left] image:ROOT:image$NVIDIA_LOGO.png[NVIDIA Logo,align=right] image:ROOT:image$LINUX_LOGO.png[LINUX Logo,align=right]
...
.^| xref:MNT-ProdToMountMap:MOUNT-MAP-PRSNT-011_ProductToMountMapping.adoc[MOUNT-MAP-PRSNT-011,window=read-later]
.^| Product-to-Mount Mapping

.^| xref:MNT-ProdToMountMap:MOUNT-MAP-PRSNT-011_ProductToMountMapping.adoc[Mounts,window=read-later]
...
.^| xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RV-ALPR-MAN-001,window=read-later]
.^| RoadView ALPR User Guide

.^| xref:IZS:IZS-S-MAN-002_Illuminator_User_Guide.adoc[IZS-S-MAN-002,window=read-later]
...
.^| xref:IZIO:IZIO-6060-MAN-001_Digital_IO_Controller.adoc[IZIO-6060-MAN-001,window=read-later]
...
* Log into each camera, and change its IP address according to the requirements of your network. +++Use fixed addresses+++. Record these new addresses for use in RoadView configuration - see <<ps_Logging-In-to-the-Camera>> and <<ps_Changing-a-Camera-s-IP-Address>> .
* Calibrate the cameras - see <<s_Calibrating-Cameras>> .
* Use IZ Discovery to find the IP address of the RoadView computer. Change the computer's IP address according to the requirements of your network. See <<s_Determining-IP-of-RoadView>> and the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
* Log in to RoadView, and configure RoadView, including adding the connected cameras - see <<ps_Using-RoadView>> and the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
* On the Journal tab, verify that Events are being generated for each vehicle passing each camera, and that the recognition has sufficient accuracy and confidence. See <<ps_Using-RoadView>> and the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
...
|Mounting Hardware (pan-tilt-roll bracket) |Typically on gantry or wall/pole; see the xref:MNT-ProdToMountMap:MOUNT-MAP-PRSNT-011_ProductToMountMapping.adoc[Mounting Hardware Documentation,window=read-later] for details.
|===
...
*The INEX {shortprodname} Camera Systems are typically used with the xref:IZS:IZS-S-MAN-002_Illuminator_User_Guide.adoc[IZS Series Strobe Illuminators,window=read-later].*
...
Secure the Camera System to the appropriate mounting hardware (see the xref:MNT-ProdToMountMap:MOUNT-MAP-PRSNT-011_ProductToMountMapping.adoc[Mounting Hardware Documentation,window=read-later]).
...
[#ps_Configuring-a-Camera]
== Configuring a Camera

*_\{Zvulun: entire section could be a partial}_*

[#ps_Logging-In-to-the-Camera]
...
[#ps_Changing-a-Camera-s-IP-Address]
...
See the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for calibration instructions.
...
. You should see the RoadView Journal tab. See the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for instructions for configuring and using RoadView.

=== Verifying the Installation

* Using a license plate mounted in a lab, or by driving a vehicle through the lane, verify that an Event is generated with the correct plate read (recorded in the RoadView Journal tab - see the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later]).
...
See the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for logout instructions, using the multi-line dropdown menu icon in the upper right corner of the screen.
...
See the xref:RoadViewALPR:OLD_RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
