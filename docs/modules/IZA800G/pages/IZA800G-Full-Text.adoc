...
.^| xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RV-ALPR-MAN-001,window=read-later]
...
* Use IZ Discovery to find the IP address of the RoadView computer. Change the computer's IP address according to the requirements of your network. See <<s_Determining-IP-of-RoadView>> and the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
* Log in to RoadView, and configure RoadView, including adding the connected cameras - see <<ps_Using-RoadView>> and the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
* On the Journal tab, verify that Events are being generated for each vehicle passing each camera, and that the recognition has sufficient accuracy and confidence. See <<ps_Using-RoadView>> and the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
...
See the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for calibration instructions.
...
. You should see the RoadView Journal tab. See the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for instructions for configuring and using RoadView.

=== Verifying the Installation

* Using a license plate mounted in a lab, or by driving a vehicle through the lane, verify that an Event is generated with the correct plate read (recorded in the RoadView Journal tab - see the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later]).
...
See the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later] for logout instructions, using the multi-line dropdown menu icon in the upper right corner of the screen.
...
See the xref:RoadView:RV-ALPR-MAN-001_RV_ALPR_User_Guide_Release_1x.adoc[RoadView ALPR User Guide,window=read-later].
