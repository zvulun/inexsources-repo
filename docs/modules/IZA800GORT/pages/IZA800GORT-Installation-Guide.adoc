...
.^|xref:IZCAB-A800-PAS:DocList.adoc[IZCABA800PAS-REV-x-TDSHEET] |Power/Signals Cable for {camera-8G} System Technical Data Sheet
.^|xref:IZCAB-A800-LAN:DocList.adoc[IZCABA800LAN-REV-x-TDSHEET] |LAN Cable for {camera-8G} System Technical Data Sheet
...
wiring (see <<s_Typical-Wiring-Diagrams>>).
