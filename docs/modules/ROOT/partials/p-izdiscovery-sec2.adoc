...
ifdef::xref-type-IZ800[<<ps_Logging-In-to-the-Camera>>]
).
...
You can also log in to each camera's configuration application to change its IP address (see <<ps_Configuring-a-Camera>>).
...
ifeval::["{shortprodname}" == "RoadView"]
<<s_Logging-In-and-Logging-Out>>
endif::[]
ifeval::["{shortprodname}" == "IZA800G"]
