...
.^| xref:IZPANEL:DocList.adoc[DIN Rail Mountable Connection Panel ({hw-connection-panel})]

.^| xref:IZ_COMPOSITE_CABLE:DocList.adoc[IZ_COMPOSITE_CABLE]

.^| Power/LAN Cable for {system-type} System+++<br>+++
(xref:IZCAB-AVES:DocList.adoc[(IZCAB-AVES)])

.^| Illuminator Cable for {system-type} System (xref:IZCAB-SVES:DocList.adoc[IZCAB-SVES])
