...
* Waterproof insulating tape (such as butyl tape) for unused camera connections
...
a|
* You must use a drip loop for the cable entering the camera
(see <<s_Install-Cameras-and-Other-Components>>).

* All cable connections from the camera to the power supply and LAN
must be enclosed in a waterproof junction box (not provided).

* Unused connections must be sealed with waterproof
insulating tape (such as butyl tape).
...
[CAUTION]
=====================================
To make a drip loop (U-shaped slack part
of the cable leaving a device that directs
moisture down and away): Adjust the camera's
base (see <<f_Pan-Tilt-Roll-Angle-Adjustments>>) so
that the notch through which the camera's cable
will pass is pointing down (see Figure <<f_Drip-Loop>>.
=====================================
...
image::ROOT:IZ600F/IZ600F-30-FIG-049_DripLoop_CableInNotch.png[image,width=800]
...
LAN must be enclosed in a waterproof junction box.

You must seal all unused connections with waterproof
insulating tape, such as butyl tape (such as the BNC connector and multi-wire cable),
