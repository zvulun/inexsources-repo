...
.^|xref:IZREMRELAY:DocList.adoc[IZREMRELAY-REV-A-TDSHEET] |{hw-webrelay-single} Datasheet
.^|xref:IZREMRELAY:DocList.adoc[{hw-webrelay-single} Series Supporting Documentation] |{hw-webrelay-single} Remote (WEB) Relay Series: Supporting Documentation
.^|xref:IZODPUG:DocList.adoc[{hw-outdoor-processor}-4G-REV-x-TDSHEET] |{hw-outdoor-processor}/4G AI Outdoor Data Processing Unit: Technical Data Sheet
...
** PoE{plus} (IEEE 802.3at) switch capable of providing 12VDC at 1.2 Amp
...
{compname-short} can be found at the end of the
...
- Other Non-reflective schemes (special colors, for example) may change the specified distances. Contact {compname-short} for advice.
...
a|
*Cable Connection Waterproofing+++*+++*

*+++<u>+++CAUTION+++</u>+++*

+++<u>+++{compname-short} products must be mounted as
described in their Installation Guides. If not, moisture
problems may occur - which are not covered by the warranty.+++</u>+++

a|
* All cable connections from the camera to the power
supply and LAN, and the power supply, must be enclosed
in a waterproof junction box (not provided).
...
The power supply type and configuration
may be different than the one at your site.
(See the
xref:s_Related-Documents[Quick Start guide]
for other wiring options.)

[#f_Typical-Wiring-Diagram]

.Typical Wiring Diagram

image::ROOT:IZ600F/IZ600F-30-FIG-047_NEWIllumQuickStartWiring.png[image,width=700]
...
* If you are using PoE{plus}, be sure that your PoE{plus} connection is compatible with IEEE 802.3at, and can provide 12 VDC at 1.2 Amp.
...
location (see <<s_Plan-Your-Site>>
and <<s_Prepare-Cables>>).
...
=== Waterproofing the Cable Connection Points (Junction Box)

[CAUTION]
========================================
All connections from the camera to the power supply and
LAN, and the power supply, must be enclosed in a waterproof junction box.
//(see <<f_Connections-In-Waterproof-Junction-Box>>).
========================================

//[#f_Connections-In-Waterproof-Junction-Box]

//.Connections in a Waterproof Junction Box
...
image::./UserGuide/image17.png[image,width=294]

. If needed, insert the rubber gasket (5) into the endcap (4). If there is a ridge, fit the rubber gasket inset ridge on the ridge inside the endcap (see <<f_Rubber-Gasket-Inset-Ridge-Fitting-on-Ridge-Inside-Endcap>>).
+
...
image::./UserGuide/image42.png[image,width=430]
