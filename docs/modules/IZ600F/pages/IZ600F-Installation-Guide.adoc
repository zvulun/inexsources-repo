...
.^|xref:IZIDPUGC:DocList.adoc[{hw-gate-controller}-REV-x-TDSHEET] |Gate Controller and Indoor AI Data Processing Unit: Technical Data Sheet
...
image::ROOT:IZ600F/IZ600F-30-FIG-047_IllumQuickStartWiring.png[image,width=474,height=198]
...
See Figure <<f_Securing-the-Waterproof-Jacket>>.
+
...
button at the end of the list of parameters.
(The Back button returns to a sub-menu
...
[NOTE]
========================================
The settings in this section are for basic
license plate recognition.
Other applications may require different settings.
========================================
...
Enter the camera's IP address in your browser,
and click on the Admin link in the
Home|Live View|Admin menu (upper right).
Use the default user/password = root/IZpass12

*Set the Access Level to Full,
with Unencrypted Authentication
(see <<s_Adding-a-User>>),
enable the RTSP Service and select a port*.
This will enable you to receive the video stream
without any special user identification;
you can define other users and their
roles/permissions later if needed.

*Set the Max. Video Encoding before setting
any other parameters (see Max. Video Encoding
in <<t_Basic-Setup-Parameters>>*.
...
+++<u>+++Changing this parameter will
require a reset, which can change some of the
other parameters. Re-check parameter values after
doing a reset.+++</u>+++
...
*It is also highly recommended to change the Administrator's password (using the System > Admin. Password parameter group) occasionally to keep the camera secure. However, if this password is lost, you will have to reset the device with the FD (Factory Default) button (see <<s_Hardware-Reset-Resets-Parameters-and-Administrator-Password>>). All setting values will be reverted to their factory defaults, and any additional user accounts that were created will be deleted.*
...
image::ROOT:IZ600F/IZ600F-30-FIG-036a_No_Callout_Access_Level_Full.png[image,width=357,height=242]
...
image::ROOT:IZ600F/IZ600F-30-FIG-017_Delete_User.png[image,width=316,height=192]

[#s_Hardware-Reset-Resets-Parameters-and-Administrator-Password]

=== Hardware Reset (Resets Parameters and Administrator Password)

In some cases (such as if the Administrator password is lost), you will need to perform a hardware reset using the FD (Factory Default) button. **All setting values will be reverted to their factory defaults, and any additional user accounts that were created will be deleted (see <<s_Adding-a-User>>)*.
