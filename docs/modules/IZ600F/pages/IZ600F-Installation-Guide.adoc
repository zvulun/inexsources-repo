...
first under dark conditions, with the Day & Night Control set to Night
(see <<s_Camera-Setup>>). If you will also have to recognize
plates under light (day) conditions,
...
|Video & Audio > Stream Setup |Frame Rate |The options available are determined by the Max. Video Encoding setting in this parameter group.
...
|Video & Audio > Camera Setup |Shutter Time Max a|1/3000; *_\{should we say this}_*
...
*_\{admin p/w has a setup menu item!!!}_*
...
and select the Access Level you need (see *_\{xref}_*)
+
...
include "Unencrypted". If you change the Authentication,
click the Apply button (you will be prompted to close the browser,
and you will have to log in again).

. Navigate to *_\{change the name}_* System > User (see <<f_User-Screen-System-User>>):
+
[#f_User-Screen-System-User]

.User Screen (*_\{change the name}_* System > User)
