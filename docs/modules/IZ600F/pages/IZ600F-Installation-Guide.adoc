...
*_\{what is the new firmware version?}_*firmware version
4.47-152-rp_ZA. Later versions may require changes to
this document.
...
with *_\{really need metal body?}_* metal-body
RJ45 connectors. The total length of the cable
should not exceed 328 feet (100 meters).
*See <<s_Prepare-Cables>>
for important LAN cable information.*

* Tools for building LAN cables (wire stripper, crimp tool, etc.) and RJ45 connectors with *_\{really need metal body?}_* metal bodies.
+
[IMPORTANT]
...
*_\{TBD – do you want to put the other
quick start wiring diagrams here?}_*
...
*_\{remove this part of the note?}_* The camera is
not compatible with some GigE switches;
suggested switch type: 10/100 Mb.
...
[table.withborders,width="100%",cols="8%,44%,26%,22%",options="header",]
|===
...
[table.withborders,width="100%",cols="18%,82%",options="header",]
|===
|No. |Component
...
include::ROOT:partial$IZ600F/p-IZ600F-connectors.adoc[IZ600F Connector Pinouts]
...
If any power cables were lengthened, ensure that all cameras receive exactly their rated voltage (see your camera's User Guide).
...
*Set the Max. Video Encoding before setting any other parameters (see Max. Video Encoding in <<t_Basic-Setup-Parameters>>. Changing this parameter will require a reset, which can change some of the other parameters and their ranges.
